...
class CycleGAN(BaseModel):
    ...
    def __init__(self, args):
        super(CycleGAN, self).__init__(args)
    ...
    def load_weights(self):
        ...
        self.netG_B2A.load_state_dict(torch.load(os.path.join("weights", str(self.args.dataset), "netG_B2A.pth")))
    ...
    def test(self, inputs):
        ...
        fake_image_A = self.netG_B2A(real_image_B).data
        fake_image_B = self.netG_A2B(real_image_A).data
    ...
    def save_test_images(self, output, norm_output, iter):
        ...
        plt.subplot(1, 3, 1), plt.imshow(real_image_A), plt.axis('off'), plt.title('A')
        plt.subplot(1, 3, 2), plt.imshow(fake_image_B), plt.axis('off'), plt.title('G(A)')
        plt.subplot(1, 3, 3), plt.imshow(diff_A2B, cmap='magma'), plt.axis('off'), plt.title('Gray|A - G(A)|')
        plt.savefig(f"{A2B_PATH}/{iter + 1:03d}.png", dpi=300, bbox_inches='tight')
        plt.clf()

        plt.subplot(1, 3, 1), plt.imshow(real_image_B), plt.axis('off'), plt.title('A')
        plt.subplot(1, 3, 2), plt.imshow(fake_image_A), plt.axis('off'), plt.title('G(A)')
        plt.subplot(1, 3, 3), plt.imshow(diff_B2A, cmap='magma'), plt.axis('off'), plt.title('Gray|A - G(A)|')
        ...
        plt.savefig(f"{B2A_PATH}/real_{iter + 1:04d}.png", dpi=300, bbox_inches='tight')
        plt.clf()
        plt.imshow(fake_image_A), plt.axis('off')
        plt.savefig(f"{B2A_PATH}/fake_{iter + 1:04d}.png", dpi=300, bbox_inches='tight')
        plt.clf()

        plt.close()        

    def metrics_evaluation(self, output):
